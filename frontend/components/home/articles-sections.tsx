        ...
        <div className="md:flex">
          <div className="flex h-48 items-center justify-center bg-gradient-to-br from-primary/10 to-accent/10 md:w-1/3">
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">{article.imageKind === "committee" ? "委員会" : "審議"}</p>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {(article.participants ?? []).slice(0, 3).map((participant) => (
                <Badge
                  ...
                  onClick={(event) => handleEvent(event, () => onParticipantClick(participant))}
                >
                  {participant}
                </Badge>
              ))}
                ...
                <div className="flex flex-wrap gap-1">
                  {(article.keywords ?? []).slice(0, 2).map((keyword) => (
                    <Badge
                      ...
                      variant="secondary"
                      className="cursor-pointer text-xs"
                      onClick={(event) => handleEvent(event, () => onKeywordClick(keyword))}
                    >
                      {keyword}
                    </Badge>
                  ))}
