...
variable "headlines_job_enabled" {
  ...
  validation {
    condition     = !var.headlines_job_enabled || var.frontend_bucket != null || var.headlines_job_bucket != null
    error_message = "headlines_job_bucket or frontend_bucket must be set when headlines_job_enabled is true."
  }
}
